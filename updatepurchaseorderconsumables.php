...
try {
    if ($conn) {
        ...
        $updateQuery = "UPDATE reorderconsumables SET PurchaseOrder = ?,Status = ?, lastupdated = ? WHERE PartNo = ?";
        ...
        if ($statement) {
            ...
            if ($result) {
                $response = "Purchase Order has been updated to $purchaseOrder for Part No: $partNo";
            } else {
